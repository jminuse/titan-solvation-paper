...
makefile = open('lammps/Makefile_sea', 'w')
makefile.write('all :')
for test_molecule_filename in ['../micelle/hcn2.arc']:
	test_molecule_name = test_molecule_filename[ test_molecule_filename.rindex('/')+1 :-4]
	if test_molecule_name[-2:] in ['e2', 'n2']: test_molecule_name = test_molecule_name[:-1]
	for step in range(0,101):
		run_name = test_molecule_name+'_100_0_0__'+str(int(step))
		makefile.write(' %s' % run_name )
makefile.write('\n\n')

#for filename in ['../micelle/propanenitrile.arc', '../N2.arc', '../acetylene.arc', '../micelle/acrylonitrile2.arc', '../micelle/acetonitrile2.arc']:

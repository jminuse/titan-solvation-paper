...
def run_dissolved(test_molecule_filename, solvent_ratio=[5,0,95]):
			...
			for z in range(-box_size[2]/2+solvent_spacing[2], box_size[2]/2, solvent_spacing[2]):
				try:
					for a in atoms[:atom_count]:
						if (a.x-x)**2 + (a.y-y)**2 + (a.z-z)**2 < 3.5**2:
	...
	for step in range(0,n_steps+1):
		...
		run_name = test_molecule_name+'_atm__'+str(int(step))
...
makefile = open('lammps/Makefile_atm', 'w')
makefile.write('all :')
for test_molecule_filename in ['../micelle/hcn2.arc']:
	test_molecule_name = test_molecule_filename[ test_molecule_filename.rindex('/')+1 :-4]
	if test_molecule_name[-2:] in ['e2', 'n2']: test_molecule_name = test_molecule_name[:-1]
...
#for filename in ['../micelle/propanenitrile.arc', '../N2.arc', '../acetylene.arc', '../micelle/acrylonitrile2.arc', '../micelle/hexane2.arc', '../micelle/acetonitrile2.arc']:
#for filename in ['../micelle/hcn2.arc']:
for filename in ['../N2.arc']:
